const util = require("prerender-memo/lib/util.js");
...
server.init = function (options) {
  ...
  this.browser = require("prerender-memo/lib/browsers/chrome.js");

  // memo
  if (server.options.ttl !== undefined) ttl = server.options.ttl;
...
// memo
server.loadSitemap = async function () {
  if (server.options.sitemap) {
    try {
      const response = await fetch(server.options.sitemap);
      if (response.ok) {
        const text = await response.text();
        const doc = new JSDOM(text);
        const pathsDoc = doc.window.document.querySelectorAll("loc");
        this.paths = new Set();
        pathsDoc.forEach((path) => {
          this.paths.add(path.textContent);
        });
        util.log("Sitemap loaded, amount paths:", this.paths.size);
      } else {
        util.log("Error HTTP:", response.status);
      }
    } catch (error) {
      util.log("Error fetch", error);
    } finally {
      let sitemapPreriod = 86400000;
      if (server.options.sitemapRefreshPreriod) {
        sitemapPreriod = server.options.sitemapRefreshPreriod;
      }
      setTimeout(this.loadSitemap.bind(this), sitemapPreriod);
    }
  }
};

// memo
server.checkSitemap = function (req, res) {
  if (this.paths?.size) {
    if (!this.paths.has(req.prerender.url)) {
      util.log("No", req.prerender.url);
      util.log("No in sitemap", this.paths);
      req.prerender.statusCode = 404;
      return Promise.reject();
    }
  }
  return this.getCache(req, res);
};

// memo
server.getCache = async function (req, res) {
...
// memo
server.setCache = function (req, res) {
  ...
  this.firePluginEvent("requestReceived", req, res)
    .then(() => {
      if (!validUrl.isWebUri(encodeURI(req.prerender.url))) {
        ...
        // memo
        return this.checkSitemap(req, res);
      }
    })
    .then(() => {
