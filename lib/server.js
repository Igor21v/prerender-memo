...
// memo
server.checkSitemap = async function (req, res) {
  if (server.options.sitemap !== undefined) {
    try {
      ...
      if (response.ok) {
        ...
        pathsDoc.forEach((path) => {
          paths.add(path.textContent);
        });
        util.log(paths.size);
        if (!paths.has(req.prerender.url)) {
          util.log("No", paths);
          util.log("No in sitemap", req.prerender.url);
